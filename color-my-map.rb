...
# Download
puts "Downloading from #{config.svg.url.last} ..."
doc = Nokogiri::XML(RestClient.get(config.svg.url.last))

# Insert description and style
eol = $/
desc = doc.create_element('desc')
desc.add_child(doc.create_cdata(eol +
                                'World map generated by Color my Map (https://github.com/freayd/color-my-map) from:' +
                                config.svg.url.to_enum(:each_with_index).collect { |url, index| "\n" + ('  ' * (index + 1) + '> ' + url) }.join +
                                eol))
style = doc.create_element('style', :type => 'text/css')
style.add_child(doc.create_cdata(eol + File.open(config.style.path).read.strip + eol))
